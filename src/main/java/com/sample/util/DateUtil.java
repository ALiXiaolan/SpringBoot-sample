...
import com.sample.exception.BaseException;
...
public class DateUtil {
    ...
    private static final ChronoField[] CHRONO_FIELDS = new ChronoField[]{ChronoField.YEAR, ChronoField.MONTH_OF_YEAR, ChronoField.DAY_OF_MONTH, ChronoField.HOUR_OF_DAY, ChronoField.MINUTE_OF_HOUR, ChronoField.SECOND_OF_MINUTE, ChronoField.MILLI_OF_SECOND};

    private static final long TIMESTAMP_THRESHOLD = 999999999999L;
    ...
    public static boolean isEqual(Date d1, Date d2, ChronoField field) {
        if (Arrays.stream(CHRONO_FIELDS).noneMatch(e -> e == field)) {
        ...
        for (ChronoField curField : CHRONO_FIELDS) {
    ...
    /**
     * 计算两个日期之间相差多少分钟，保留2位小数
     *
     * @param startDate
     * @param endDate
     * @return
     */
    public static double calcMinuteDiff(Date startDate, Date endDate) {
        double result = 0;
        long begin = startDate.getTime();
        long end = endDate.getTime();
        long diff = end - begin;
        long unitMillis = ChronoUnit.MINUTES.getDuration().toMillis();
        if (diff > 0) {
            result = diff / ((double) unitMillis);
        }
        return result;
    }
